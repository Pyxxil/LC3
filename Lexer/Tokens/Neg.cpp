...
void Neg::assemble(int16_t &programCounter, size_t width,
  ...
  const uint16_t bin = 0x903F | (DR << 9) | (SR << 6);

  set_assembled(AssembledToken(
      bin, fmt::format("({0:0>4X}) {1:0>4X} {1:0>16b} ({2: >4d}) {3: "
                       "<{4}s} NOT R{5:d} "
                       "R{6:d}",
                       programCounter++, bin, line(), sym, width, DR, SR)));

  const uint16_t bin1 = 0x1021 | (DR << 9) | (SR << 6);
  ...
  as_assembled.emplace_back(
      ...
      fmt::format(
          ...
          "R{6:d} #1",
          programCounter++, bin1, line(), std::string{}, width, DR, SR));
}
#endif
} // namespace Lexer::Token