...
void Jsrr::assemble(uint16_t &program_counter, size_t width,
                    const std::map<std::string, Symbol> &symbols,
                    const std::string &sym) {
  const auto reg = static_cast<Register *>(operands().front().get())->reg();
  const auto bin = static_cast<uint16_t>(OP_JSR | reg << 6);
