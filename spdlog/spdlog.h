...
// exist. example: spdlog::get("my_logger")->info("hello {}", "world");
//
...
// async_logger. effective only for loggers created after this call. queue_size:
// size of queue (must be power of 2):
...
//    has room for the new log entry. async_overflow_policy::discard_log_msg -
...
// Basic logger simply writes to given file without any limitations or
// rotations.
//
...
std::shared_ptr<logger> android_logger(const std::string &logger_name,
...
create_async(const std::string &logger_name, const sink_ptr &sink,
             size_t queue_size,
             const async_overflow_policy overflow_policy =
                 async_overflow_policy::block_retry,
             const std::function<void()> &worker_warmup_cb = nullptr,
...
std::shared_ptr<logger>
create_async(const std::string &logger_name, sinks_init_list sinks,
             size_t queue_size,
             const async_overflow_policy overflow_policy =
                 async_overflow_policy::block_retry,
             const std::function<void()> &worker_warmup_cb = nullptr,
...
template <class It>
std::shared_ptr<logger>
create_async(const std::string &logger_name, const It &sinks_begin,
             const It &sinks_end, size_t queue_size,
             const async_overflow_policy overflow_policy =
                 async_overflow_policy::block_retry,
             const std::function<void()> &worker_warmup_cb = nullptr,
             const std::chrono::milliseconds &flush_interval_ms =
...
// statements. Uncomment SPDLOG_DEBUG_ON/SPDLOG_TRACE_ON in tweakme.h to enable.
